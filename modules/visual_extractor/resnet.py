...
class NormLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = torch.matmul(normed_x, F.normalize(self.weight, dim=0))
        outputs.clamp_(-1, 1)
        return outputs * 16
...
class I3Res50(nn.Module):
    ...
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=400, use_nl=False, stride=4, duration=8):
        self.stride = stride
    ...
    def forward_video(self, x):
        temporal, channel, height, width = x.size()
        left = self.duration // 2
        right = temporal // self.stride * self.stride + self.duration // 2 - temporal
        x_pad = torch.zeros(left + temporal + right, channel, height, width, device=x.device)  # [t+ws, c, h, w]
        x_pad[left:left + temporal] = x
        split_list = []
        len_list = list(range(left + temporal + right))
        for t in range(left, left + temporal, self.stride):
        ...
        segment = 12 // self.stride
        features, preds = [], []
        for i in range(segment - 1):
            feature, pred = self.forward_clip(x_list[i*(temporal//segment):(i+1)*(temporal//segment)])
            features.append(feature)
            preds.append(pred)
        feature, pred = self.forward_clip(x_list[(segment-1)*(temporal//segment):])
        features.append(feature)
        preds.append(pred)
    ...
    def forward(self, inputs, inputs_type='clip'):
        inputs = inputs.transpose(1,2)
        if inputs_type == 'clip':
            feature = self.forward_clip(inputs)

        elif inputs_type == 'video':
            feature, pred = self.forward_video(inputs)
...
def i3_res50(num_classes=-1):
    net = I3Res50(num_classes=num_classes, use_nl=False)
    state_dict = torch.load(
        '/home/jiaopeiqi/CrossView-SLR/checkpoints/I3D/i3d_r50_kinetics.pth', map_location=torch.device('cpu'))
...
def i3_res50_nl(num_classes=-1):
    net = I3Res50(num_classes=num_classes, use_nl=True)
    state_dict = torch.load(
        '/home/jiaopeiqi/CrossView-SLR/checkpoints/I3D/i3d_r50_nl_kinetics.pth', map_location=torch.device('cpu'))
    del state_dict['fc.bias']
    del state_dict['fc.weight']
    net.load_state_dict(state_dict, strict=True)
    # freeze_bn(net, "net") # Only needed for finetuning. For validation, .eval() works.
