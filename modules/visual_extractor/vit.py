...
class VisionTransformer(nn.Module):
    ...
    def forward_video(self, x):
        b,c,t,h,w = x.size()
        steps = (t-self.duration) // self.stride + 1
        video_segmentations = []
        for start in range(0, t-self.duration+1, self.stride):
            video_segmentations.append(x[:,:,start:start+self.duration,:,:])
        chunks = len(video_segmentations)
        features = []
        for i in range(chunks):
            features.append(self.forward_features(video_segmentations[i]))
        split_features = torch.stack(features, dim=1)
...
def vit_base_patch16_224(pretrained=False, **kwargs):
    ...
    weight = torch.load('checkpoints/VIT/vit_b_k710_dl_from_giant.pth', map_location=torch.device('cpu'))
    model.load_state_dict(weight['module'], strict=True)
